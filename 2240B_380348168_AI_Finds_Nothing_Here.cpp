// Codeforces Problem: 2240B - AI Finds Nothing Here
// Submission ID: 380348168
...
// Problem: B_AI_Finds_Nothing_Here
// Rating: Rating
// =============================================================
...
void solve() {
    ...
    cin >> n >> m >> r >> c;
     ll M = MOD - 1 ;
     ll total = ((n%M) * (m%M)) % (M) ;
     ll row = max(0LL , n - r +1) ;
    ll col = max(0LL , m - c + 1) ;
     ll sq = ((row%M) * (col%M)) % (M) ;
     cout << power(2 ,(total - sq + M) % (M))  << '\n' ;
}
 int main() {
    fast;
    int t = 1;
