// Codeforces Problem: 1790D - Matryoshkas
// Submission ID: 379318866
...
ll modMul(ll a, ll b) { return ((a % MOD) * (b % MOD)) % MOD; }
 void solve() {
    ...
    int prev_freq = 0;
        if (freq.count(ele - 1)) {
            prev_freq = freq[ele - 1];
        }
                cnt += max<int>(0, fr - prev_freq);
    }
     // a.erase(unique(all(a)), a.end()) ; // remove dupl
    // int cnt = freq[a[0]] ;
     // rep(i,1,sz(a))
    // {
    //     if(a[i]==a[i-1] + 1) {
    //         if(freq[a[i]]>freq[a[i-1]])
    //             cnt += freq[a[i]] - freq[a[i-1]] ; // freq[a-i] ne already consumekrliyaa
    //     }
     //     else
    //         cnt += freq[a[i]] ;
    // }
