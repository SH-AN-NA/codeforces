...
void setadder(int n , vector<int>&arr)
{
    for(int i = 0 ; i < 32 ; i++) 
        arr[i] += ((n >> i) & 1);
        // arr[i] += (n & (1<<i)) ;  jyada gand mt chalao
        // adds 2^i istead of 1
    
}
void solve() {
    int n ; cin >> n ;
    vector<int> a(n) ,comsets(32,0);
    
    for0(i,n)
    {
        cin >> a[i] ;
        setadder(a[i] , comsets) ;
    }
    for1(i,n)
    {   
        bool ok = true ;
...
// and == 1 0 0 0 0
 
// 1 0 1 0 1 - and == 101(removes common setbit compleyely)
 
 
int main() {
    fast;
